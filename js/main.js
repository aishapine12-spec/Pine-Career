  ...
  (function densifyGlobeDots(){
    ...
    for (var i = 0; i < pts.length; i++){
      for (var j = i + 1; j < pts.length; j++){
        ...
        if (dist > 0 && dist < GAP){
          extra.push([ (pts[i][0] + pts[j][0]) / 2, (pts[i][1] + pts[j][1]) / 2 ]);
        }
      }
    }
    GLOBE_DATA.dots = pts.concat(extra);
  })();
...
function createGlobe(canvas, opts){
  ...
  /*
   * ============================================================
   * ATMOSPHERE LIMB GLOW (bright edge scatter, fully inside the
   * canvas so it never gets clipped  adds a realistic, premium
   * "planet with atmosphere" rim brightening).
   * ============================================================
   */
  ...
  function drawAtmosphere(){
    ...
    g.addColorStop(0, "rgba(130,182,221,0)");
    g.addColorStop(0.72, "rgba(130,182,221,0.08)");
    g.addColorStop(1, "rgba(196,220,240,0.34)");
  ...
  function drawSphereShading(){
    ...
    /*
     * Fine gold hairline just inside the rim  a subtle
     * "premium metallic" edge accent.
     */

    ctx.beginPath();
    ctx.arc(cx, cy, R - 3, 0, Math.PI * 2);
    ctx.strokeStyle = "rgba(228,200,131,0.16)";
    ctx.lineWidth = 1;
    ctx.stroke();


    /*
     * Glossy specular highlight, upper-left  gives the
     * sphere a polished, three-dimensional sheen.
     */
